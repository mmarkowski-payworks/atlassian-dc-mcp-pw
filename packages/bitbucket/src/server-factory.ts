...
export function createBitbucketServer(service: BitbucketService, version = '0.0.0-test') {
  const server = createMcpServer({ name: 'atlassian-bitbucket-mcp', version });
  ...
  server.tool(
    ...
    async ({ projectKey, repositorySlug, pullRequestId, text, parentId, filePath, line, lineType, pending, output }) => {
      const result = await service.postPullRequestComment(projectKey, repositorySlug, pullRequestId, text, parentId, filePath, line, lineType, pending, output);
      return formatToolResponse(result);
    }
  );
  ...
  server.tool(
    ...
    async ({ projectKey, repositorySlug, title, description, fromRefId, toRefId, reviewers, output }) => {
      const result = await service.createPullRequest(projectKey, repositorySlug, title, description, fromRefId, toRefId, reviewers, output);
      return formatToolResponse(result);
    }
  );
  ...
  server.tool(
    ...
    async ({ projectKey, repositorySlug, pullRequestId, version, title, description, reviewers, output }) => {
      const result = await service.updatePullRequest(projectKey, repositorySlug, pullRequestId, version, title, description, reviewers, output);
      return formatToolResponse(result);
    }
  );
